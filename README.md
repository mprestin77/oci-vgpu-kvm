...
7.2 Once vGPU license is registered with Nvidia vGPU software license server you can confirm that by running running "nvidia-smi -q" on a Windows client VM. By default, nvidia-smi.exe is installed in Windows in c:\Program Files\NVIDIA Corporation\NVSMI folder. Look for "License status" in the output of the command
